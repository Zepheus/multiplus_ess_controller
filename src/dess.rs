...
impl FeedInExcess {
    ...
    /// Is feed-in disabled this tick? Mirrors the stock ESS loop's ``: for `Follow`
    /// (0) defer to the user's `PreventFeedback` setting; else 1 ⇒ disabled, 2 ⇒ enabled.
    pub fn feedin_disabled(self, prevent_feedback: bool) -> bool {
        match self {
            FeedInExcess::Follow => prevent_feedback,
            FeedInExcess::Disable => true,
            FeedInExcess::Enable => false,
        }
    }
}
...
impl DessOut {
    ...
    /// Whether feed-in is disabled this tick, given the user's `PreventFeedback` setting
    /// (only consulted on the `Follow`/0 branch). Convenience over `feedin()`.
    pub fn feedin_disabled(&self, prevent_feedback: bool) -> bool {
        self.feedin().feedin_disabled(prevent_feedback)
    }
}
...
mod tests {
    ...
    #[test]
    fn feedin_disabled_decision() {
        // Follow(0) defers to the user's PreventFeedback; 1/2 are absolute ().
        assert!(!FeedInExcess::Follow.feedin_disabled(false)); // user allows feed-in
        assert!(FeedInExcess::Follow.feedin_disabled(true)); // user prevents feed-in
        assert!(FeedInExcess::Disable.feedin_disabled(false)); // forced off
        assert!(!FeedInExcess::Enable.feedin_disabled(true)); // forced on
    }
